        ...
        Object.keys(filterd_classes).forEach((key0) => {
                        ...
                        Object.keys(filterd_strata).forEach((key2) => {
                            ...
                            Object.keys(filterd_elements).forEach((key3) => {
                                let filterd_blocks = blocks?.current["LC_Block"].filter( 
                                    block => block.block_id === filterd_elements[key3]["BlockID"] 
                                );
                                
                                // Get the instanceIndex of the current property being processed
                                let currentInstanceIndex = filterd_elements[key3]["instanceIndex"] || 0;
                                
                                // Filter characteristics by StratumID, BlockID, AND instanceIndex
                                let filterd_xteristics = LC_Characteristics?.current.filter( characteristics => 
                                    characteristics.StratumID === filterd_strata[key2]["stratumID"] && 
                                    characteristics.BlockID === filterd_blocks[0]["block_id"] &&
                                    (characteristics.instanceIndex || 0) === currentInstanceIndex
                                );
            ...
            classes = [...classes, 
                                [
                                    ...
                                    [
                                        [
                                            `A`+class_id+` -.-o B`+class_id+`[[fa:fa-gears Class Characteristics:`+DisplayClassCharacteristics+` ]]`
                                        ],
                                        ...patterns
                                    ] 
                                ]
...
return ( 
    <Viewer chart={flowDefinition} legendValid={legendValid} setActiveLCElement={setActiveLCElement} />
 );
};